...
const InfoBar = ({ notifications, className, role, subAccountId }: Props) => {
    ...
    const handleClick = () => {
        if (!showAll) {
            ...
            if (notifications?.length !== 0) {
                setAllNotifications(notifications?.filter((notification) => notification.subAccountId === subAccountId) ?? []);
            }
        }
    ...
    return (
        <>
            <div
                className={
                    twMerge('fixed z-[20] md:left[300px] left-0 right-0 top-0 p-4 bg-background/80 backdrop-blur-md flex gap-4 items-center border-b-[1px]', className)}>
                                ...
                                <div className='flex gap-2'>
                                    <Avatar>
                                        <AvatarImage
                                            ...
                                            alt='profile picture'
                                        />
                                        <AvatarFallback className='bg-primary'>{notification.users.name.slice(0, 2).toUpperCase()}</AvatarFallback>
                                    </Avatar>
